...
CallbackReturn ManipulatorInterfaceSTS3215::on_init(const hardware_interface::HardwareInfo &hardware_info)
{
  ...
  {
    loadServoIds(servo_ids_it->second, info_.joints.size());
  }
                                                                  ...
                                                                  const rclcpp::Duration &)
{
  ...
  bool any_read = false;
  size_t count = std::min(servo_ids_.size(), position_states_.size());
                                                                   ...
                                                                   const rclcpp::Duration &)
{
  ...
  size_t count = std::min(servo_ids_.size(), position_commands_.size());
  for (size_t i = 0; i < count; ++i)
  {
