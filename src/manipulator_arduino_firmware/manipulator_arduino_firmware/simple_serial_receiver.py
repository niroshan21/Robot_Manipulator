...
class SimpleSerialReceiver(Node):
    ...
    def __init__(self):
        ...
        self.frequency_ = 0.01  # frequency_ means the timer interval in seconds that checks for new serial data

        self.arduino_ = serial.Serial(port=self.port_, baudrate=self.baudrate_, timeout=0.1)
        
        self.timer_ = self.create_timer(self.frequency_, self.timerCallback)  # Create a timer to call timerCallback at the specified frequency

    def timerCallback(self): # Callback function does the serial data reading and publishing
